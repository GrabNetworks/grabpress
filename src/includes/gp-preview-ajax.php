...
?>
<div id="gp-catalog-container" style="display:none;">
	<form method="post" action="" id="form-preview-page">
	<div class="wrap" >
		<fieldset id="preview-feed">
		<legend>Insert Video</legend>		
			<div class="label-tile-one-column">
				<span class="preview-text-catalog"><b>Keywords: </b><input name="keywords" type="text" value="<?php echo $keywords = isset($form['keywords']) ? $form['keywords'] : '' ?>" maxlength="255" /></span>
			...
			<div class="label-tile">
				...
				<div class="tile-right">
					...
					<select name="channel[]" id="preview-channel-select" class="channel-select multiselect" multiple="multiple" style="width:500px" >
						<?php	
			...
			<div class="label-tile">
				...
				<div class="tile-right">
					<select name="provider[]" id="preview-provider-select" class="multiselect" multiple="multiple" style="<?php GrabPress::outline_invalid() ?>" onchange="doValidation()" >
					<?php			
			...
			<div class="label-tile">
				...
				<div class="tile-right">
					From<input type="text" value="<?php echo $created_after = isset($form['created_after']) ? $form['created_after'] : ''; ?>" maxlength="8" name="created_after" class="datepicker" />					
				</div>
			</div>	
			<div class="label-tile">	
				To<input type="text" value="<?php echo $created_before = isset($form['created_before']) ? $form['created_before'] : ''; ?>" maxlength="8" name="created_before" class="datepicker" />
				<div class="tile-right">					
	...
	<?php $qa = GrabPress::$environment == 'grabqa'; ?>
		( function ( global, $ ) {
			...
			global.hasValidationErrors = function () {
				if(($("#preview-channel-select :selected").length == 0) || ($("#preview-provider-select :selected").length == 0)){
					return true;
				}
				else {
					return false;
		...
		jQuery(function($){	
			...
			if($('#preview-provider-select option:selected').length == 0){
				$('#preview-provider-select option').attr('selected', 'selected');
			}

			if($('#preview-channel-select option:selected').length == 0){
				$('#preview-channel-select option').attr('selected', 'selected');
			}
			$("#preview-channel-select").multiselect(multiSelectOptionsChannels, {
		   ...
		   $("#preview-provider-select").multiselect(multiSelectOptions, {
		  	 uncheckAll: function(e, ui){
		  	 	doValidation();
			 },
			 checkAll: function(e, ui){
		   ...
		   $(".datepicker").datepicker({
			...
			var submitSearch = function(){
		   		var data = { "action" : "get_catalog", 
		   					 ...
		   					 "providers" : $("#preview-provider-select").val(),
		   					 "channels" : $("#preview-channel-select").val(),
		   ...
		   $("#form-preview-page").change(doValidation);
		   $("#form-preview-page").submit(function(e){
		   		e.preventDefault();
		   		submitSearch();
		   		return false;
		   });
		   $(".sort_by").change(function(e){
			...
			//populate and update search w/data
			$from = $("#form-create-feed");
			$to = $("#form-preview-page");
			$(':input[name]', $from).each(function() {
		        $('[name=' + $(this).attr('name') +']', $to).val($(this).val());
		    });
		    submitSearch();
